...
def build_prompt(system_prompt: str, rag_results: List[Dict], kernel_log: str, user_query: str, max_chunks=1):
    ...
    rag_results = sorted(rag_results, key=lambda x: x["score"])[:max_chunks]
...
def rag_query(request: QueryRequest):
    ...
    rag_result_raw = vs.similarity_search_with_score(f"{request.query}\n\n{request.kernel_log}", k=request.max_results)
